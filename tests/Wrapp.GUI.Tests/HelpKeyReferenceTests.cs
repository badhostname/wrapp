...
public class HelpKeyReferenceTests
{
    ...
    // ------------------------------------------------------------------
    // 2b. A section or view guide is never a tooltip. A HelpKey opens as a
    //     rendered document; the same key as a ToolTip dumps the whole
    //     guide, headings and tables included, into a hover popup (1.0.7:
    //     the Detection expression box). Fields get their own short key.
    // ------------------------------------------------------------------

    private static readonly Regex GuideKeyRx = new(
        "HelpKey=\"(?<key>Help\\.[^\"]+)\"", RegexOptions.Compiled);

    private static readonly Regex TooltipKeyRx = new(
        "ToolTip=\"\\{StaticResource (?<key>Help\\.[^}\\s]+)\\}\"", RegexOptions.Compiled);

    [Fact]
    public void NoSectionGuide_IsUsedAsATooltip()
    {
        var guides = new HashSet<string>(StringComparer.Ordinal);
        var tooltips = new Dictionary<string, List<string>>(StringComparer.Ordinal);
        var files = Directory.EnumerateFiles(GuiSrcDir(), "*.xaml", SearchOption.AllDirectories)
            .Where(f => !f.Contains($"{Path.DirectorySeparatorChar}obj{Path.DirectorySeparatorChar}")
                     && !f.EndsWith("HelpContent.xaml", StringComparison.OrdinalIgnoreCase));
        foreach (var file in files)
        {
            var text = File.ReadAllText(file);
            foreach (Match m in GuideKeyRx.Matches(text)) guides.Add(m.Groups["key"].Value);
            foreach (Match m in TooltipKeyRx.Matches(text))
            {
                var key = m.Groups["key"].Value;
                if (!tooltips.TryGetValue(key, out var list)) tooltips[key] = list = new List<string>();
                list.Add(Path.GetFileName(file));
            }
        }
        var offenders = tooltips
            .Where(kv => guides.Contains(kv.Key))
            .Select(kv => $"{kv.Key} (tooltip in {string.Join(", ", kv.Value.Distinct())})")
            .OrderBy(s => s)
            .ToList();
        Assert.True(offenders.Count == 0,
            "Section/view guides (HelpKey=) used as ToolTips - give the field its own short key:\n"
            + string.Join("\n", offenders));
    }
