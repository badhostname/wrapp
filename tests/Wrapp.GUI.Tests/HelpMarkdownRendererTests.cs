...
public class HelpMarkdownRendererTests
{
    ...
    [Fact]
    public void PreprocessLegacyFieldLines_TokenLabel_IsNotRewritten()
    {
        // A paragraph that starts with a button/badge token carries a colon
        // inside the token; it is not a "Label: value" line.
        var output = HelpMarkdownRenderer.PreprocessLegacyFieldLines("[btn:Filter] opens the conditions dialog.");

        Assert.DoesNotContain("**[btn:**", output);
        Assert.Contains("[btn:Filter] opens the conditions dialog.", output);
    }
