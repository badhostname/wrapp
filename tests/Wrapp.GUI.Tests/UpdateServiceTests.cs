...
public class UpdateServiceTests
{
    ...
    // ------------------------------------------------------------------
    // GitHub feeds. A release page is not a browsable directory, so these
    // must route to the releases API rather than the static web source.
    // ------------------------------------------------------------------

    [Theory]
    [InlineData("https://github.com/badhostname/wrapp", "https://github.com/badhostname/wrapp")]
    [InlineData("https://github.com/badhostname/wrapp/", "https://github.com/badhostname/wrapp")]
    [InlineData("https://github.com/badhostname/wrapp/releases", "https://github.com/badhostname/wrapp")]
    [InlineData("https://github.com/badhostname/wrapp/releases/tag/v1.0.4", "https://github.com/badhostname/wrapp")]
    [InlineData("https://www.github.com/badhostname/wrapp", "https://github.com/badhostname/wrapp")]
    public void GitHubFeed_TrimsToRepoRoot(string input, string expected)
    {
        Assert.True(UpdateService.TryGetGitHubRepoUrl(input, out var repo));
        Assert.Equal(expected, repo);
    }

    [Theory]
    [InlineData("https://updates.contoso.com/wrapp")]      // static directory feed
    [InlineData("https://github.com/badhostname")]          // owner only, no repo
    [InlineData("http://github.com/badhostname/wrapp")]     // not HTTPS
    [InlineData(@"\\fileserver\wrapp\releases")]            // UNC
    [InlineData(@"C:\wrapp\releases")]                      // local folder
    public void NonGitHubFeed_IsNotTreatedAsRepo(string input)
        => Assert.False(UpdateService.TryGetGitHubRepoUrl(input, out _));

    [Fact]
    public void GitHubFeed_StillRequiresApprovalAndHttps()
    {
        const string repo = "https://github.com/badhostname/wrapp";
        Assert.True(UpdateService.IsValidFeedUrl(repo));
        Assert.False(UpdateService.IsFeedTrusted(repo, storedToken: null));
        Assert.True(UpdateService.IsFeedTrusted(repo, UpdateService.IssueFeedTrustToken(repo)));
    }
}
