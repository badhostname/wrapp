...
public class SourceLintTests
{
    ...
    // --------------------------------------------------------------------
    // Rule: Add-IntuneWin32AppFromConfig's $HandledKeys covers every
    // package key the function reads itself.
    //
    // The function transforms some package keys (Notes -> tracking JSON,
    // Comment -> Description, IconFile -> Icon, ...) into the defaults it
    // hands to Get-ValidatedWin32AppParameters, then forwards every OTHER
    // package key verbatim as an override. A key that is both read here and
    // forwarded overwrites its own transformed value. 1.0.6 shipped that
    // for Notes: the raw (empty) package note replaced the CreatedBy/Guid/
    // Date JSON, was stripped as blank, and every assignment was skipped
    // with "GUID mismatch".
    // --------------------------------------------------------------------

    private static readonly Regex PackageReadRegex = new(
        @"\$Package\.(?<key>[A-Za-z_][A-Za-z0-9_]*)|Get-ConfigValue\s+\$Package\s+'(?<key>[A-Za-z_][A-Za-z0-9_]*)'",
        RegexOptions.Compiled);

    [Fact]
    public void PackagerHandledKeysCoverEveryPackageRead()
    {
        var target = Path.Combine(
            RepoRoot, "modules", "Wrapp.Packager", "Public", "Add-IntuneWin32AppFromConfig.ps1");
        if (!File.Exists(target)) return; // skip when module not present

        var source = File.ReadAllText(target);
        var handledBlock = Regex.Match(source, @"\$HandledKeys\s*=\s*@\((?<body>[\s\S]*?)\n\s*\)");
        Assert.True(handledBlock.Success, "Add-IntuneWin32AppFromConfig.ps1: $HandledKeys = @( ... ) not found.");
        var handled = Regex.Matches(handledBlock.Groups["body"].Value, @"'(?<k>[A-Za-z_][A-Za-z0-9_]*)'")
            .Select(m => m.Groups["k"].Value)
            .ToHashSet(StringComparer.OrdinalIgnoreCase);

        var read = new SortedSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (var rawLine in source.Split('\n'))
        {
            if (rawLine.TrimStart().StartsWith("#", StringComparison.Ordinal)) continue;
            foreach (Match m in PackageReadRegex.Matches(rawLine))
            {
                var key = m.Groups["key"].Value;
                if (key == "PSObject") continue; // the merge loop itself
                read.Add(key);
            }
        }

        // Anchors: the rule is only meaningful if the scan sees the reads
        // the function is known to make.
        Assert.Contains("Notes", read);
        Assert.Contains("Comment", read);
        Assert.Contains("PackageOption", read);

        var missing = read.Where(k => !handled.Contains(k)).ToList();
        Assert.True(missing.Count == 0,
            "Add-IntuneWin32AppFromConfig.ps1 reads these package keys but does not list them "
            + "in $HandledKeys, so the raw package value is forwarded as an override and "
            + "replaces the transformed one: " + string.Join(", ", missing));
    }
