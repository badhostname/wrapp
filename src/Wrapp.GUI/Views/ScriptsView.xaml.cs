...
public partial class ScriptsView : UserControl
{
    ...
    /// <summary>
    /// Expands {{Name}} placeholders in the ACTIVE tab's
    ...
    private async void HistoryButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            // A draft whose folder is gone (or a bundle moved on disk): say
            // so instead of letting git init throw from the click handler.
            vm.StatusMessage = "The bundle's Script folder is missing on disk. Save the bundle first.";
            AppLogger.Warn($"Scripts: history requested but the Script folder is missing: {repoDir}");
            return;
        }
