...
public partial class MainWindow : FluentWindow
{
    ...
    /// <summary>
    /// THE close pipeline (Phase C): every close entry point — the X button,
        ...
        SettingsViewModel  settingsVm)
    {
        ...
        // Phase C: one close pipeline. Every gate — background jobs, running
        ...
        // Perf-plan P2.4: pause the bundle dirty-check serializer while the
    ...
    /// <summary>
    /// Update-handoff close (Phase D): the same CloseGuard walk as any close
    ...
    private async void JobsButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        // Header row: intro blurb + "Clear completed" link
    ...
    /// <summary>Hamburger: toggles the sidebar between labeled rows (150px)
    /// and icons only (48px — the NavBadgeStack style stacks badges
    /// vertically over the icon's right edge in that state, so the rail
    /// stays narrow with both badges visible). The NavItem/NavLabel styles
    /// react to <see cref="MainViewModel.IsNavCollapsed"/> for label
    /// visibility; the column width is the only view-side concern.</summary>
    private void NavCollapse_Click(object sender, RoutedEventArgs e)
    {
        _vm.IsNavCollapsed = !_vm.IsNavCollapsed;
        SidebarColumn.Width = new GridLength(_vm.IsNavCollapsed ? 48 : 150);
    }
