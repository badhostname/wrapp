...
public partial class MainWindow : FluentWindow
{
    ...
    private async Task RunCloseAsync(CloseReason reason)
    {
        try
        {
            ...
            _closeConfirmed = true;
            // Back-ported from Unwrapped: with no dirty scopes and no jobs the
            // guard completes SYNCHRONOUSLY, so without a yield this Close()
            // re-enters inside the original Closing dispatch and throws
            // InvalidOperationException ("Cannot ... Close ... while a Window
            // is closing") - leaving the window permanently unclosable.
            // Yielding lets the first Closing event unwind first.
            await Task.Yield();
            Close();
        }
        finally { _closePending = false; }
