...
# MSI + MSP Patch - Installs {{MSIFile}} then applies a patch file

$MSIFile = "{{MSIFile}}"
$MSPFile = "{{MSIFile}}" # Update this to the .msp filename
$BinDir  = Join-Path -Path $PSScriptRoot -ChildPath "..\B"

$MSIPath = Join-Path -Path $BinDir -ChildPath $MSIFile
$MSPPath = Join-Path -Path $BinDir -ChildPath $MSPFile

# Install base MSI
if (Test-Path $MSIPath) {
    $Args = @('/i', "`"$MSIPath`"", '/qn', '/norestart', 'ALLUSERS=1')
...
if (Test-Path $MSPPath) {
    $Args = @('/update', "`"$MSPPath`"", '/qn', '/norestart')
    $Proc = Start-Process -FilePath 'msiexec.exe' -ArgumentList $Args -Wait -PassThru
}
