...
# Uninstall Previous Version + MSI Install

$AppName = "{{Name}}"
$MSIFile = "{{MSIFile}}"
$BinDir  = Join-Path -Path $PSScriptRoot -ChildPath "..\B"

# Find and uninstall previous versions from registry
$UninstallPaths = @(
...
foreach ($Path in $UninstallPaths) {
    Get-ItemProperty -Path $Path -ErrorAction SilentlyContinue |
        ...
        ForEach-Object {
            ...
            Write-Host "Uninstalling previous: $($_.DisplayName) ($ProductCode)"
            Start-Process 'msiexec.exe' -ArgumentList "/x `"$ProductCode`" /qn /norestart" -Wait
        }
}

# Install new MSI
$InstallerPath = Join-Path -Path $BinDir -ChildPath $MSIFile
if (Test-Path $InstallerPath) {
    $Args = @('/i', "`"$InstallerPath`"", '/qn', '/norestart', 'ALLUSERS=1')
