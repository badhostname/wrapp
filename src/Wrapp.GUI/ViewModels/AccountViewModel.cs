...
public partial class AccountViewModel : StatusViewModelBase
{
    ...
    private async Task ApplyTokenResultAsync(MsalTokenResult result)
    {
        ...
        // run has its own dedicated runspace pool, so switching identity
        ...
        // makes mid-run identity switches safe.
    }
    ...
    // MUST be the SAME path MsalAuthService writes - PlatformConfig owns the
    // resolution (env override + appsettings.json included). A hand-derived
    // copy here once read a file nobody writes under WRAPP_MSAL_CACHE_PATH.
