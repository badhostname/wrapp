...
public partial class RunViewModel : StatusViewModelBase
{
    ...
    /// <summary>
    /// B′ Stage 2: true while the active pass's progress is driven by the
    /// module's typed <c>Write-WrappStep</c> events (the Intune path), which
    /// causes <see cref="OnPhaseChanged"/> to suppress the equivalent regex
    /// phases. Cleared for the SCCM path, whose module still reports progress
    /// via log text (regex) until Workstream F. Read on PS streaming threads,
    /// written on the run thread -- volatile for visibility.
    /// </summary>
    ...
    /// <summary>
    /// Phase 16b. Wires the feature-gate so <see cref="CaptureEncryptionKeys"/>
    ...
    private async Task StartAsync()
    {
        ...
        // Populate package progress items from the loaded config
        ...
        try
        {
            ...
            // Warn about any past dates in assignments/deployments
            WarnPastDates();
            ...
            if (Mode is "PackageOnly" or "FullRun")
            {
                ...
                {
                    // Acquire tokens for enabled tenants
                    await AcquireAndInjectTokenAsync();

                    // Run once per enabled tenant with packages filtered to that tenant
                    ...
                    {
                        // Workstream P: the tenant loop lives in the MODULE
                        // (Invoke-WrappPackaging groups packages by their config
                        // TenantId and runs one pass per tenant), so UI and CLI
                        // share the same orchestration path. The UI contributes:
                        //   - its enabled/connected subset as the -TenantIds filter
                        //   - one MSAL token per enabled tenant as an injected map
                        //     (each pass promotes its own entry; no cross-tenant reuse)
                        // Per-pass progress scoping now comes from the module's
                        // TenantPass boundary events (see ApplyStepEvent), not
                        // from a .NET loop iteration.
                    ...
                    else
                    {
                        ...
                        foreach (var conn in enabledSccmSites)
                        {
                            ...
                            var sccmTargeted = sccmPackages
                                .Where(p => p.IsEnabled                      // operator intent
                            ...
                            // let the regex phases drive it (Workstream F).
        ...
        catch (OperationCanceledException)
        {
            ...
            // Mark running packages as cancelled
    ...
    private void PopulatePackageProgress()
    {
        ...
        if (Target is RunTarget.Intune or RunTarget.Both)
        {
            // Collect enabled tenant IDs for skip detection
            var enabledTenants = IntuneConnections
                .Where(c => c.IsEnabled)
                .ToList();
            ...
            // Build the target label: "Intune - TenantName" when a single tenant
            ...
            foreach (var pkg in config.Script.IntunePackager.Packages)
            {
                // Determine if this package has an enabled tenant target
                ...
                // Per-package label: show the targeted tenant name
