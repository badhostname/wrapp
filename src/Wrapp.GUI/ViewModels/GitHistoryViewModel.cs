...
public partial class GitHistoryViewModel : StatusViewModelBase
{
    ...
    public GitHistoryViewModel(GeneralViewModel generalVm)
    {
        ...
        _generalVm.ConfigLoaded += (_, _) =>
        {
            ...
            if (_viewVisible && !_pollTimer.IsEnabled)
                _pollTimer.Start();
            // STA-3 (2026-07 audit): ConfigLoaded is a domain event that may be
            // raised off the UI thread, so an async-void handler's fault would
            // escape to AppDomain.UnhandledException (terminating). Route the
            // refresh through SafeFireAndForget, which try/catches + logs.
    ...
    private async Task PollForChangesAsync()
    {
        ...
        try
        {
            ...
            // Refresh the commit list if the latest hash changed
    ...
    private async Task ViewCommitAsync(CommitInfo? commit)
    {
        ...
        // Use pre-loaded file changes; fall back to fetching if empty
