...
public partial class ScriptsViewModel : ObservableObject
{
    ...
    /// <summary>
    /// Bundle directory containing the scripts (parent of Config.json).
    /// Derived from GeneralViewModel's live config path, never cached: a
    /// first save moves a draft out of the temp workspace (which is then
    /// deleted), and a copy taken at load time would keep pointing at it.
    /// The save hook cannot be relied on to refresh a copy either, as it
    /// returns early when the editor was never opened.
    /// </summary>
    public string ConfigDir => Path.GetDirectoryName(_generalVm.ConfigPath) ?? string.Empty;
    ...
    private async Task OnBundleSavingAsync()
    {
        ...
        // ConfigDir already reflects the save target (GeneralViewModel re-points
        // its config path before raising BundleSaving).
        var scriptDir = ConfigDir;
    ...
    /// folder (ConfigDir), but PSADT's Invoke-AppDeployToolkit.ps1 lives at the bundle root.
    /// </summary>
    private string? ResolveScriptPath(string fileName)
    {
        ...
        var scriptPath = Path.Combine(configDir, fileName);
