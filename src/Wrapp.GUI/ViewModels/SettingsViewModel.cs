...
public partial class SettingsViewModel : ObservableObject
{
    ...
    /// <summary>
    /// Phase 16a. Opt-in flag for the Azure DevOps key vault feature gate.
    /// When unchecked, the URL TextBox visibly disables and every vault
    /// touchpoint short-circuits via <see cref="Services.IFeatureGate"/>.
    /// Persisted to <see cref="AppSettings.EnableAzureDevOpsKeyVault"/> on save.
    /// </summary>
    [ObservableProperty]
    private bool _enableAzureDevOpsKeyVault;

    /// <summary>
    /// Phase 16c. Single-brace path template for auto-captured vault keys.
    /// Tokens documented in <see cref="Services.VaultPathTemplate.SupportedTokens"/>.
    /// The <see cref="KeyVaultPathPreview"/> computed property renders a
    /// live example against fixture data so the operator can confirm the
    /// shape before saving.
    /// </summary>
    [ObservableProperty]
    ...
    /// <summary>
    /// Phase 16c. Single-brace path template for manually-saved vault keys
    /// (no Tenant / AppId available). Defaults to <c>/manual/{PackageName}.json</c>
    /// to preserve the pre-16c layout.
    /// </summary>
    [ObservableProperty]
    ...
    /// <summary>
    /// Phase 16d. PR-mode opt-in. When true, vault pushes create a feature
    /// branch + Pull Request instead of committing directly to main. The
    /// PR template fields (<see cref="KeyVaultPrSourceBranchTemplate"/>,
    /// <see cref="KeyVaultPrTitleTemplate"/>, <see cref="KeyVaultPrDescriptionTemplate"/>)
    /// only appear in the UI when this flag is checked.
    /// </summary>
    [ObservableProperty]
    private bool _keyVaultUsePullRequests;

    /// <summary>
    /// Phase 16d. PR source branch template. Same tokens as
    /// <see cref="KeyVaultPathTemplate"/>. Defaults group branches by date.
    /// </summary>
    [ObservableProperty]
    ...
    /// <summary>
    /// Phase 16d. PR title template. Same tokens as <see cref="KeyVaultPathTemplate"/>.
    /// </summary>
    [ObservableProperty]
    ...
    /// <summary>
    /// Phase 16d. PR description template. Same tokens as
    /// <see cref="KeyVaultPathTemplate"/>. Multi-line; newlines are
    /// preserved through the resolve.
    /// </summary>
    [ObservableProperty]
    private string _keyVaultPrDescriptionTemplate;

    /// <summary>
    /// Workstream D. Update feed (HTTPS URL or UNC share). Approval on Save
    /// follows the same TOFU pattern as the Key Vault URL; the manual check
    /// below runs against the SAVED value, not unsaved edits.
    /// </summary>
    [ObservableProperty]
    private string _updateFeedUrl;

    /// <summary>Workstream D. "Auto" | "NotifyOnly" | "Disabled".</summary>
    [ObservableProperty]
    ...
    /// <summary>Phase 16d. Live preview of the resolved PR source branch.</summary>
    ...
    /// <summary>Phase 16d. Live preview of the resolved PR title.</summary>
    ...
    /// <summary>
    /// Phase 16c. Live preview of <see cref="KeyVaultPathTemplate"/> resolved
    /// against fixture data. Re-renders whenever the template field changes
    /// so the operator sees the exact path that will land in the vault.
    /// </summary>
    ...
    /// <summary>
    /// Phase 16c. Live preview of <see cref="KeyVaultManualPathTemplate"/>
    /// resolved against the same fixture data, with empty Tenant / AppId
    /// so the manual case shows the path the user actually gets.
    /// </summary>
    ...
    // Stable fixture so the live preview is deterministic across renders.
    // Plausible-looking values are chosen on purpose -- a value like
    // "example-tenant" reads as "this is a placeholder" rather than "is
    // this a real GUID?". Kept private so the fixture can change shape
    // without touching public surface.
    ...
    /// <summary>
    /// True when the current in-memory Settings differ from the baseline
    /// snapshot taken at load/save. Mirrors the diff-based pattern used by
    /// <see cref="GeneralViewModel.CheckForChanges"/> for the bundle — if the
    /// user reverts an edit (e.g. removes a tenant then clicks Restore
    /// defaults), IsDirty clears automatically with no explicit reset path.
    /// </summary>
    [ObservableProperty]
    ...
    /// <summary>
    /// Path-bar text for the Settings section. Mirrors the title-bar
    /// <see cref="MainViewModel.StatusBarPath"/> format exactly: the raw path
    /// with a trailing " *" when there are unsaved changes. Shown in the save
    /// strip's left slot so the operator always knows where the preferences
    /// live on disk.
    /// </summary>
    ...
    /// <summary>
    /// Raw path to settings.json (no dirty indicator) for the dedicated
    /// path row at the top of <c>SettingsView</c>. Distinct from
    /// <see cref="PreferencesBarText"/> so the in-panel display stays clean
    /// even when unsaved-change indicators are elsewhere.
    /// </summary>
    ...
    /// <summary>
    /// Workstream P (P2): the Placeholders tab — live built-in values, custom
    /// placeholder editing, effective-configuration viewers and the
    /// log-redaction summary. Persists through <see cref="SaveAsync"/>.
    /// </summary>
    public PlaceholdersViewModel Placeholders { get; }

    /// <summary>
    /// Pass-through of <see cref="PlaceholdersViewModel.ErrorCount"/> so
    /// <see cref="MainViewModel.SettingsErrorCount"/> can relay it to the
    /// Settings nav badge (same trickle-back as the assignment error counts).
    /// Change notification is wired in the constructor via
    /// <see cref="PropertyRelay"/>.
    /// </summary>
    ...
    public SettingsViewModel(AppSettings settings, TenantsViewModel tenantsVm)
    {
        ...
        // Perf-plan P2.4: this tick serializes the full preferences +
        // placeholders state (three JSON passes) — the heaviest always-on
        // work in the app, and its inputs can only change while the Settings
    ...
    private void CheckForChanges()
    {
        ...
        {
            // BUG-3: same latch as GeneralViewModel — a persistent fault here
    ...
    private void RestoreSavedTenantsCore()
    {
        ...
        if (_settings.IntuneTenants.Count > 0)
        {
            ...
            {
                // Config already has tenants -- merge settings into matching entries
    ...
    /// <summary>
    /// Public access to <see cref="DefaultDomains"/> for
    /// <see cref="PreferencesViewModel"/>'s first-ever load of the Preferences
    /// Domains table. Mirrors <see cref="DefaultTenants"/> / <see cref="DefaultSites"/>.
    /// </summary>
    ...
    /// <summary>
    /// Workstream D: manual update check from the Settings page. Runs against
    /// the SAVED feed URL (unsaved edits require Save first, which also runs
    /// the approval prompt). Check-only; a found update reveals the
    /// "Install update" button (<see cref="UpdateFoundVersion"/>).
    /// </summary>
    /// <summary>
    ...
    private async Task ImportOrgDefaultsAsync()
    {
        ...
        // Same refresh set the other load paths use: scalars
        // (ReloadFromSettings, as ImportSettingsAsync), the Preferences tables
        // (LoadFromSettings — whose empty-settings fallback is what surfaces
        // the imported tenants/sites/domains), and the bundle's tenant lists
        // (RestoreSavedTenants, as startup/ConfigLoaded do).
        ReloadFromSettings();
    ...
    /// <summary>
    /// Phase D: hands off to the splash-level update flow — normal save
    /// prompts first (Cancel aborts and keeps the session), then the update
    /// screen downloads, installs, and relaunches on the new version.
    /// </summary>
    [RelayCommand]
    ...
    {
        // Workstream P polish: placeholder rows in error (duplicate / reserved /
        // invalid names) BLOCK the whole save — nothing is persisted until the
        // flagged rows are fixed (user chose disallow over skip).
        ...
        // issuing a machine-bound trust token. SEC-1 (2026-07 audit): the token
        // is a DPAPI envelope of the URL (not a plain hash), so an attacker who
        // can only write settings.json cannot forge approval.
        ...
        // Workstream D: same TOFU shape for the update feed. The feed delivers
        // executable code, so an unapproved URL is never contacted -- declining
        // here just leaves the token unissued (checks report FeedNotApproved).
        ...
        // Workstream P: placeholders persist through the SAME save click.
        // Sensitive values are DPAPI-encrypted into the sidecar here; on
        // encryption failure the placeholder rows are left un-persisted (the
        // user was warned) while the rest of the settings still save.
        ...
        // Phase 16a. AppSettings is a plain POCO, so the gate service doesn't
        // observe the property mutations above. Fire an explicit notification
        // so any bound consumer (toolbar buttons, badge text) re-evaluates
        // IsEnabled without the user having to restart Wrapp.
    ...
    /// <summary>
    /// Phase 16a. Wires the feature-gate service so <see cref="SaveAsync"/>
    /// can fire <see cref="Services.IFeatureGate.NotifyChanged"/> after the
    /// gate-relevant flags are copied back into <see cref="AppSettings"/>.
    /// Called from <c>App.xaml.cs</c> alongside the existing wire-up of
    /// DevOps auth.
    /// </summary>
    ...
    public void WireDevOpsAuth(Services.DevOpsAuthService devOpsAuth)
    {
        ...
        // Check if already signed in from cache
        _ = Task.Run(async () =>
        {
    ...
    private async Task ResetDefaultsAsync()
    {
        ...
        // Phase 16c/16d: reseed every key-vault template from a fresh
        // AppSettings instance so the defaults stay defined in exactly one
        // place (the model). Avoids duplicating the literal default strings
        // here every time a new template lands.
    ...
    /// </list>
    /// Replaces a recurring 13-16 line pattern across the four
    /// <c>Reset*</c> commands. The complex
    /// <see cref="ResetAllSettingsAsync"/> command does NOT use this helper
    /// &#x2014; its file-move + try/catch + multi-stage in-place reload is
    /// genuinely different shape and forcing it through the helper would
    /// hurt readability.
    /// </summary>
    ...
    private async Task ResetAllSettingsAsync()
    {
        ...
        try
        {
            // BUG-2: PlatformConfig owns this path (env override included) —
            // the same class already reads it that way elsewhere.
    ...
    {
        // Captured outside the lambda so the post-reset success dialog can
        // surface the actual count. successMessage is a Func so the
        // interpolation runs AFTER reset() has populated the captured value.
        int count = 0;
