...
public static class SmoothScroll
{
    ...
    private static void OnPreviewMouseWheel(object sender, MouseWheelEventArgs e)
    {
        ...
        // A wheel over an open dropdown list belongs to the list alone. This
        // TUNNEL handler on the page viewer runs before the list's own
        // scroller sees the event; scrolling the page here would slide it out
        // from under the open popup (which cannot follow), even when the list
        // itself has nothing left to scroll.
        if (ScrollBubbling.WheelIsOverOpenDropDown(e)) return;
