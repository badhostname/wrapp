...
{
    /// <summary>
    /// Workstream O: one-shot flag -- true once OrgDefaultsSeeder has run for
    ...
    // Workstream P: custom {{Name}} placeholders. Sensitive rows keep Value
    ...
    /// <summary>
    /// Phase 16a (FeatureGate). When false, every Azure DevOps key vault
    ...
    /// <summary>
    /// Phase 16c. Path template for auto-captured vault keys. Tokens:
    /// <c>{Tenant}</c>, <c>{AppId}</c>, <c>{AppName}</c>, <c>{PackageName}</c>,
    /// <c>{Date}</c>, <c>{Author}</c>. Default preserves the pre-16c layout
    ...
    /// <summary>
    /// Phase 16c. Path template for manually-saved vault keys (no
    ...
    /// <summary>
    /// Phase 16d. When true, vault pushes create a feature branch + Pull
    ...
    /// <summary>
    /// Phase 16d. Source branch name template for PR mode. Same tokens as
    ...
    /// <summary>
    /// Phase 16d. PR title template. Same tokens as
    ...
    /// <summary>
    /// SEC-1 (2026-07 audit). Machine-bound TOFU trust token for the approved
    ...
    // =======================================================================
    // Gate framework (2026-07): required user actions surfaced after a code /
    ...
    // =======================================================================
    // Workstream D (2026-07): installed-app update channel (Velopack).
    // =======================================================================
    ...
    /// <see cref="KeyVaultRepoUrlHash"/> (SEC-1), issued by
...
/// <summary>
/// Workstream P: one custom <c>{{Name}}</c> placeholder. When
...
/// <summary>
/// DPAPI helpers for encrypting/decrypting secrets at rest in settings.json.
/// Values are stored as "dpapi:BASE64" so we can distinguish them from legacy plaintext.
/// Only the current Windows user can decrypt (DataProtectionScope.CurrentUser).
/// </summary>
/// <summary>
...
public static class SecretProtection
{
    ...
    /// <summary>
    /// SEC-1 / SEC-8 (2026-07 audit). Strict variant of <see cref="Decrypt"/>
    ...
    public static SecureString? DecryptToSecureString(string? stored)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                // SEC-8 (2026-07 audit): a non-DPAPI secret in settings.json is
    ...
    /// <summary>
    /// Phase 15 (S-6). Resolves a tenant's ClientSecret into a fresh
