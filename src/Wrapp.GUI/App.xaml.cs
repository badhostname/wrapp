...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        // scrolling page: a wheel over the list scrolls the list, any scroll
        // of the page around it closes the list (see ScrollBubbling).
        ...
        // Every launch is a full instance -- the old exit-and-foreground gate
        // is gone. Shared state is instance-safe: per-bundle exclusive lock,
        // cross-process RMW locks on settings / sidecar / template manifest,
        // per-instance log files, per-instance WebView2 user-data folders,
        // plus the pre-existing MSAL cache mutex and temp-workspace locks.
        // The mutex remains
        ...
        // Truthful instance registry (pid lock) + close-request
    ...
    protected override void OnExit(ExitEventArgs e)
    {
        ...
        // Drop the cross-instance bundle lock (deletes its file). A hard
        // kill skips this harmlessly -- the orphaned file is reclaimable.
        BundleLockService.Release();

        // Drop the pid registration (deletes its file too). Then, if
    ...
    /// <summary>
    /// Splash probe: one fast check-only feed query, 8s budget so a
    ...
    private async Task StartupCoreAsync()
    {
        ...
        // Freezes self-document (stall monitor); startup phases get perf numbers.
        ...
        // Launch guard: never start an old binary while Update.exe is
        ...
        // Enable single-click editing for all DataGrid text cells
            ...
            // Org provisioning. Sensitive-pattern scrubbing is
            ...
            // One call wires BOTH the placeholder resolver and
            ...
            // Race a check-only feed probe against the user's card
            ...
            window.Closed += (_, _) =>
            {
                // Update handoff: the process must SURVIVE this close so the
            ...
            // If the primary settings.json was corrupt at
            ...
            var gateService = new GateService(settings, new IAppGate[]
            {
                ...
                // An available update surfaces HERE - the indicator -
            ...
            // LastRunVersion as it was BEFORE this launch records itself --
            ...
            SafeFireAndForget.Run(async () =>
            {
                ...
                // What's-New popup on the first launch after a
