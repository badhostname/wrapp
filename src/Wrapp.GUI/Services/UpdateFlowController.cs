...
/// <summary>
/// Phase D (update-flow-and-token-polling-plan): owns the splash-level update
...
/// (Phase B). Velopack applies and relaunches.</para>
/// </summary>
public static class UpdateFlowController
{
    ...
    public static async Task RunSplashFlowAsync(Views.SplashWindow splash, AppSettings settings)
    {
        ...
        try
        {
            ...
            // set now), so launches during the apply window wait — Phase B.
            System.Windows.Application.Current.Shutdown();
        }
        catch (Exception ex)
        {
