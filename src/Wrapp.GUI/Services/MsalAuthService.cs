...
public sealed partial class MsalAuthService : IDisposable
{
    ...
    public async Task<MsalTokenResult?> TryAcquireTokenSilentAsync()
    {
        ...
        try
        {
            var result = await Task.Run(async () =>
            {
                ...
                return await ForConfiguredTenant(app.AcquireTokenSilent(DelegatedScopes, existing))
        ...
        string tenantId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // The token for THIS tenant, not the configured one: this path
            // serves any tenant (inventory, status cards) without re-initialising.
            return new SilentTokenOutcome(ToResult(result, tenantId), SilentAttemptOutcome.Success);
        }
    ...
    public async Task<MsalTokenResult> AcquireTokenForAccountAsync(IAccount account)
    {
        ...
        try
        {
            ...
            result = await ForConfiguredTenant(app.AcquireTokenSilent(DelegatedScopes, account))
                .ExecuteAsync();
        }
        catch (MsalUiRequiredException)
        {
