...
public sealed class PowerShellService : IDisposable
{
    ...
    /// <para>
    /// The live IPCA was retired from the runspace; the new
...
";
    ...
    private RunspacePool CreatePool(int maxRunspaces = 3)
    {
        ...
        // Register the Invoke-WrappTokenRefresh
...
";
        ...
        Action<PSObject>? onOutput = null)
    {
        ...
        List<Guid> InjectTokens(PowerShell ps)
        {
            ...
            if (msalToken is not null)
            {
                ...
                {
                    // The registered refresh handle must be
    ...
    /// <summary>
    /// Runs the module-owned multi-tenant Intune orchestrator
        ...
        Action<PSObject>? onOutput)
    {
        ...
        // after the run regardless of how the pipeline ends.
        var msalRefreshHandles = injectTokens?.Invoke(ps) ?? new List<Guid>();

        ps.AddScript(script);

        // Route every PS stream through AppLogger.Redact
...
";
        ...
        string script, Models.MsalTokenResult token)
    {
        return await Task.Run(() =>
        {
            ...
            // Pass the token's REAL expiry, not a
