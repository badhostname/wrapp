...
public sealed partial class MsalAuthService
{
        ...
        IPublicClientApplication app, IAccount? loginHintAccount, Prompt prompt)
    {
        ...
        // Pinned to the configured tenant: the sign-in must land in the
        // tenant the run targets, not wherever the picked account is at home.
        var builder = ForConfiguredTenant(app.AcquireTokenInteractive(DelegatedScopes))
