...
public class EncryptionKeyStoreService
{
    ...
    public async Task SaveKeysAsync(EncryptionKeyInfo keys)
    {
        ...
        // Route to PR mode when the user has opted in. For
    ...
    public async Task<List<(EncryptionKeyInfo Key, string RelativePath)>> LoadAllDevOpsKeysAsync()
    {
        ...
        // The TOFU trust check guards READS too, not
    ...
    /// <summary>
    /// Issues a machine-bound trust token for
    ...
    /// <summary>
    /// Returns true only when <paramref name="storedToken"/> is an
    ...
    public static bool IsKeyVaultUrlTrusted(string? url, string? storedToken)
    {
        ...
        // Machine-mandated vault URL bypasses per-user TOFU - HKLM
    ...
    public sealed class DevOpsVaultNotConfiguredException : Exception
    {
        ...
        /// <summary>
        /// Built from <see cref="IFeatureGate.DescribeWhyDisabled"/>
    ...
    private async Task PushToDevOpsAsync(EncryptionKeyInfo keys, bool isManual = false)
    {
        ...
        // Path is operator-configurable via two settings templates
        ...
        // A genuinely malformed response (schema change,
    ...
    /// <summary>
    /// PR-mode counterpart to <see cref="PushToDevOpsAsync"/>:
    ...
    private async Task PushAsPullRequestAsync(EncryptionKeyInfo keys, bool isManual = false)
    {
            ...
            $"{apiBase}/refs?filter=heads/main&api-version=7.0"))
        {
            ...
            // An unexpected response shape is logged, not swallowed.
            catch (Exception ex)
            {
    ...
    // Read path uses the legacy hard-coded layout as a fast
    ...
    {
        // Guard reads with the same trust check as pushes.
        if (!IsKeyVaultUrlTrusted(_settings.KeyVaultRepoUrl, _settings.KeyVaultRepoUrlHash))
        {
