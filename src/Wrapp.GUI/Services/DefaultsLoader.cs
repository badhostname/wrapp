...
public static class DefaultsLoader
{
    ...
    {
        // ONLY defaults.local.json is authoritative. The example
...
public class OrgDefaults
{
    ...
    // ---- Nullable = "not specified in the file" ----
    ...
    /// <summary>Update feed URL + mode for installed Wrapp.</summary>
    public OrgUpdateDefaults? Update { get; set; }

    /// <summary>
    /// Org-defined custom placeholders. Seeded add-missing
...
/// <summary>
/// Org defaults for the update channel. Seeding the feed URL
/// does NOT approve it - each technician still confirms the URL once (the
/// trust-token gate), so a tampered defaults file cannot silently point
