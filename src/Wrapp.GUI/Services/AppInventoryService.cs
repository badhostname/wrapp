...
";
    ...
    public async Task<string?> FetchIconBase64Async(string tenantId, string appId)
    {
        ...
        // Bind appId in a SINGLE-quoted PS literal (where Escape's doubled-single-quote
        // is the correct escaping) and expand the variable into the URI, instead of
        // interpolating single-quote-escaped text into a DOUBLE-quoted string
        // (wrong metacharacter set).
        var script = $@"
$header = $Global:AuthenticationHeader
$appId = '{Escape(appId)}'
