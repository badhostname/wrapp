...
///
/// <para>Seeds the Preferences editor and the new-package AddPackage path
/// when <c>settings.json</c> has no user-customised values. The PowerShell
/// module still reads Defaults.psd1 at runtime; this is a parallel copy so
/// the GUI works without a live PS runspace.</para>
///
/// <para><b>Keep this file in sync with Defaults.psd1.</b> If you change a
/// default there, mirror it here. The duplication is intentional; drift only
/// risks a slightly-wrong seed the user immediately overrides on first save.</para>
/// </summary>
public static class ModuleDefaultsSeed
{
    ...
    // is true on an assignment). Mirrors Defaults.psd1 and the IntuneWin32App
    // cmdlet fallbacks, plus a sensible 4-hour snooze window that's otherwise
    // silently disabled.
