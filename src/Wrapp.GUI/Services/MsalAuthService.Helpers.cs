...
public sealed partial class MsalAuthService
{
    ...
    /// <summary>True when the tenant is a real tenant id (a GUID), as opposed to empty / "organizations" discovery.</summary>
    internal static bool IsSpecificTenant(string? tenantId) => Guid.TryParse(tenantId, out _);

    /// <summary>
    /// True when a token came back for a different tenant than the one asked
    /// for. The public client uses the multi-tenant authority, so an
    /// unscoped request against a cached account is answered from that
    /// account's HOME tenant whatever tenant the caller configured - which
    /// is how two placeholder tenants once showed "Connected" with the real
    /// tenant's token (1.0.7). Empty on either side means "unknown", not a
    /// mismatch.
    /// </summary>
    internal static bool IsTenantMismatch(string? requestedTenant, string? actualTenant) =>
        IsSpecificTenant(requestedTenant)
        && !string.IsNullOrEmpty(actualTenant)
        && !string.Equals(requestedTenant, actualTenant, StringComparison.OrdinalIgnoreCase);

    /// <summary>The token for the CONFIGURED tenant (the InitializeAsync one).</summary>
    private MsalTokenResult ToResult(AuthenticationResult result) => ToResult(result, _tenantId, adoptDiscoveredTenant: true);

    /// <summary>
    /// Wraps an MSAL result as the token for <paramref name="requestedTenant"/>.
    /// A result from another tenant is refused: a token is only ever
    /// reported under the tenant it is actually for. When the request was
    /// unscoped (empty tenant: the /organizations sign-in that discovers
    /// where the user landed) the discovered tenant becomes the configured
    /// one, and that is the only time the configured tenant moves.
    /// </summary>
    private MsalTokenResult ToResult(AuthenticationResult result, string? requestedTenant, bool adoptDiscoveredTenant = false)
    {
        var actualTenant = result.TenantId;
        if (IsTenantMismatch(requestedTenant, actualTenant))
        {
            AppLogger.Warn($"MSAL returned a token for tenant {actualTenant} when tenant {requestedTenant} was requested ({result.Account?.Username}); refusing it.");
            throw new TenantMismatchException(requestedTenant!, actualTenant!, result.Account?.Username);
        }

        if (!string.IsNullOrEmpty(actualTenant) && adoptDiscoveredTenant && !IsSpecificTenant(_tenantId))
        {
            AppLogger.Info($"Tenant from auth result: {actualTenant}");
            _tenantId = actualTenant;
        }
        ...
        var tokenResult = new MsalTokenResult(
            ...
            TenantId: !string.IsNullOrEmpty(actualTenant) ? actualTenant
                : IsSpecificTenant(requestedTenant) ? requestedTenant! : _tenantId,
            ClientId: _clientId,
