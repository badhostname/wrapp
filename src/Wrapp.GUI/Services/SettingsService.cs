...
public static class SettingsService
{
        ...
        string?                   endpointLocalAppFolder = null)
    {
        ...
        try
        {
            mappedTenants = intuneTenants.Select(t => new SavedTenantEntry
            {
                ...
                //
                // The fresh value is a SecureString rather
        ...
        try
        {
            ...
            // Settings.json + the user-defaults sidecar are written under
    ...
    {
        // Serialized against concurrent instance saves so the primary/.bak
    ...
    private static AppSettings LoadCore()
    {
        ...
        // Isolate the primary read+parse in its OWN try so
        ...
        if (File.Exists(SettingsPath))
        {
            try
            {
                // A settings.json in the MB range is corrupt, not big.
    ...
    public static void Save(AppSettings settings)
    {
        ...
        {
            var json = JsonSerializer.Serialize(settings, WriteOptions);
            // Whole write cycle under the cross-process settings lock.
