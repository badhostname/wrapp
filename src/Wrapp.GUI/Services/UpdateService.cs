...
/// <summary>
/// Continuous updates for installed (Velopack-packaged) Wrapp.
...
///   trust token (same pattern as the Key Vault URL) before it is ever
...
public static class UpdateService
{
    ...
    /// starting the old binary mid-swap.
    /// </summary>
    ...
    // -----------------------------------------------------------------------
    // Feed URL validation + trust tokens
    // -----------------------------------------------------------------------
    ...
    /// <summary>
    /// A GitHub repository root such as <c>https://github.com/owner/repo</c>.
    /// GitHub releases are not a browsable directory, so these feeds resolve
    /// through the releases API rather than the plain web source; see
    /// <see cref="CreateManager"/>. A deep link into /releases or
    /// /releases/download is accepted and trimmed back to the repo root,
    /// because that is what an operator naturally copies from the browser.
    /// </summary>
    internal static bool TryGetGitHubRepoUrl(string? url, out string repoUrl)
    {
    ...
    /// <summary>DPAPI trust token for an approved feed URL.</summary>
    ...
    /// <summary>
    /// Picks the source that matches the feed's shape:
    /// <list type="bullet">
    /// <item><description>UNC share or local folder -> file source.</description></item>
    /// <item><description>GitHub repo URL -> releases API. A release page is not
    ///   a directory, so the plain web source cannot read it; the API lists the
    ///   releases and their assets instead. No access token is passed, so the
    ///   repository must be PUBLIC - a private one would need a stored PAT, and
    ///   Wrapp does not hold provisionable secrets for the update path.</description></item>
    /// <item><description>Any other HTTPS URL -> static directory serving
    ///   releases.win.json beside the packages.</description></item>
    /// </list>
    /// </summary>
    private static UpdateManager CreateManager(string feedUrl)
    {
        ...
        if (TryGetGitHubRepoUrl(trimmed, out var repoUrl))
    ...
    /// <summary>
    /// The launch-time check. An available update NEVER
    ...
    /// to the splash on a sibling-free launch. Disabled mode / not
