...
public class DevOpsAuthService
{
    ...
    // QLT-2 (2026-07 audit): removed the unused parallel status API
    // (IsAuthenticated / Username / TokenExpiresUtc / TokenAcquired event) --
    // grep-verified zero subscribers; the VMs bind the MsalAuthService
    // equivalents. SetParentWindowFunc is retained (called from App.xaml.cs,
    // a no-op for the browser-based DevOps flow).

    // Not used for browser-based flow but kept for API compat
