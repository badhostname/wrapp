function Connect-WrappIntune {
    ...
    #
    # P0 hardening: the short-circuit requires the injected token to belong to
    ...
    # still-valid token for tenant B -- the cross-tenant clobber class the
    # run-scoped pool work (B'') exists to prevent. A mismatched leftover token
    ...
    switch ($AuthFlow.ToLowerInvariant()) {
        ...
        'clientcert' {
            ...
            # Search both certificate stores in a single pipeline
