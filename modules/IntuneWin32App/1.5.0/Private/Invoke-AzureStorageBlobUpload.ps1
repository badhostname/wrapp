function Invoke-AzureStorageBlobUpload {
    ...
    for ($Chunk = 0; $Chunk -lt $ChunkCount; $Chunk++) {
        ...
        if ($TokenExpireMinutes -le 10) {
            ...
            # Try injected MSAL refresh handle first (Wrapp GUI flow -- silent, no disk I/O).
            #
            # Phase 12 hardening (S-3, version 0.6.0.0232). The live MSAL
            # IPCA is no longer pushed into PowerShell scope. The handle is
            # an opaque GUID; the C# Invoke-WrappTokenRefresh cmdlet
            # registered in the runspace looks the IPCA up from a private
            # registry and performs AcquireTokenSilent in-process.
            if ($Global:WrappMsalRefreshHandle) {
                try {
                    Write-Verbose -Message "Token expiring, attempting silent refresh via MSAL handle..."
                    $authResult = Invoke-WrappTokenRefresh -Handle $Global:WrappMsalRefreshHandle

                    if ($authResult -and $authResult.AccessToken) {
                        $Global:AccessToken.access_token = $authResult.AccessToken
                        $Global:AccessToken.AccessToken = $authResult.AccessToken
                        $Global:AccessToken.ExpiresOn = $authResult.ExpiresOn
                        $Global:AuthenticationHeader['Authorization'] = 'Bearer ' + $authResult.AccessToken
                        $Global:AuthenticationHeader['ExpiresOn'] = $authResult.ExpiresOn
                        Write-Verbose -Message "Token refreshed via MSAL handle (expires $($authResult.ExpiresOn))"
                        $tokenRefreshed = $true
                    }
                }
                catch {
                    Write-Verbose -Message "MSAL silent refresh failed: $_"
                }
            }

            # Fallback to module's own refresh
