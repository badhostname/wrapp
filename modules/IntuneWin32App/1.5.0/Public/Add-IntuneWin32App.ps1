            ...
            if ($null -ne $IntuneWinXMLMetaData) {
                ...
                if ($Win32MobileAppRequest.'@odata.type' -notlike "#microsoft.graph.win32LobApp") {
                    ...
                    if ([string]::IsNullOrEmpty($Win32MobileAppContentVersionRequest.id)) {
                        ...
                        $IntuneWinFileName = [System.IO.Path]::GetFileName($FilePath)
                            ...
                            if ([string]::IsNullOrEmpty($Win32MobileAppFileContentRequest.id)) {
                                ...
                                switch ($CommitFileRequest.uploadState) {
                                    ...
                                    default {
                                        ...
                                        # Rename intunewin file to match the original file name
                                        Write-Verbose -Message "Renaming intunewin file to: $IntuneWinFileName"
                                        $Win32AppFileRenameBody = [ordered]@{
                                            "@odata.type" = "#microsoft.graph.win32LobApp"
                                            "fileName"    = $IntuneWinFileName
                                        }
                                        $Win32AppFileRenameBodyRequest = Invoke-MSGraphOperation -Patch -APIVersion "Beta" -Resource "deviceAppManagement/mobileApps/$($Win32MobileAppRequest.id)" -Body ($Win32AppFileRenameBody | ConvertTo-Json)
