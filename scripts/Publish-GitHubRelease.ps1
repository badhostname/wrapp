...
$assets = @(
    ...
    (Join-Path $releasesDir "WrappApp-$Version-full.nupkg")
)
foreach ($a in $assets) {
    if (-not (Test-Path $a)) { throw "Missing asset: $a. Run scripts\Publish-Release.ps1 first." }
...
gh release view $tag --repo $Repo --json assets --jq '.assets[] | "\(.name)  \(.size) bytes  \(.state)"'
