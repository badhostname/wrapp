...
# Format in PowerShell rather than jq: 5.1 mangles the embedded double
# quotes a jq program needs when handing arguments to a native exe, and
# gh then sees the fragments as extra positional arguments.
$view = Invoke-Probe gh @('release', 'view', $tag, '--repo', $Repo, '--json', 'assets')
if ($view.ExitCode -ne 0) {
    Write-Warning "Could not verify assets: $($view.Output)"
} else {
    foreach ($a in ($view.Output | ConvertFrom-Json).assets) {
        Write-Note ("{0,-34} {1,8:N1} MB  {2}" -f $a.name, ($a.size / 1MB), $a.state)
    }
}
