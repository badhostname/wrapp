<#
.SYNOPSIS
    Builds a distributable Wrapp release (Setup.exe + MSI +
...
if ($prefix -match '^\d+\.\d+\.\d+\.\d+$') {
    throw "VersionPrefix '$prefix' is 4-part; Velopack requires SemVer2."
}
...
# Read/write through .NET with an explicit UTF-8 encoding. Get-Content on
# 5.1 decodes a BOM-less file as ANSI, which mangled every non-ASCII
# character into mojibake inside the release manifest (seen in 1.0.3's
# embedded notes); Set-Content -Encoding utf8 would also add a BOM.
$changelog = [System.IO.File]::ReadAllLines(
    (Join-Path $repoRoot 'CHANGELOG.md'), [System.Text.Encoding]::UTF8)
...
[System.IO.File]::WriteAllText(
    $notesPath, ($sectionLines -join "`n"), (New-Object System.Text.UTF8Encoding $false))
